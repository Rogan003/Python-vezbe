...
icon = pg.image.load('iconCorona.png')
coronaImage = pg.image.load('corona2.jpg')
...
class Habits:
    ...
    def __init__(self, Mask, Gloves, InfectionChance):
        self.Mask = Mask
        self.Gloves = Gloves
    ...
    def putMask(self):
        if self.Mask == False:
            self.Mask = True
            if self.InfectionChance > 40:
                self.InfectionChance -= 40
        return self.Mask
        
    def takeOfMask(self):
        if self.Mask == True:
            self.Mask = False
        if self.InfectionChance < 60:
            self.InfectionChance += 40
        return self.Mask

    def putGloves(self):
        if self.Gloves == False:
            self.Gloves = True
            if self.InfectionChance > 20:
                self.InfectionChance -= 20
        return self.Gloves     

    def takeOfGloves(self):
        if self.Gloves == True:
            self.Gloves = False
        if self.InfectionChance < 80:
            self.InfectionChance += 20
        return self.Gloves
...
class Citizen:
    ...
    def sleepcheck(self):
        ...
        global stopClick, globaltimer, numinfected, numdead, numsurvived, population
        ...
        """
        timerSeconds = time.perf_counter()

        if int(timerSeconds - globaltimer > graphsecs:
           # show_plot(numinfected, numdead, numsurvived, timer, population)
           globaltimer = timer

        """

        if time.perf_counter() - globaltimer > graphsecs:
            globaltimer = time.perf_counter()
            print(int(globaltimer))
        ...
        infc = random.randint(1,100)
        if self.num > 0:
            self.num -= 1
        if self.num == 0 and self.color == healthycolor and not self.survived and \
        ...
        simScreen.returnTitle().get_at([self.x + 1, self.y - 1]) == infectedcolor or \
        simScreen.returnTitle().get_at([self.x - 1, self.y - 1]) == infectedcolor or \
        simScreen.returnTitle().get_at([self.x, self.y]) == infectedcolor) and \
        infc < self.habits.InfectionChance:
            self.num = 5
    ...
    def __init__(self):
        ...
        self.habits = Habits(returnBoolRand(),returnBoolRand1(),100)
...
def returnBoolRand():
    global textBoxGloves
    if not textBoxGloves.returnValue() == "":
...
def returnBoolRand1():
    global textBoxMasks
    if not textBoxMasks.returnValue() == "":
        x = random.randint(0,1)
        if x == 0:
            return True
        else:
...
menuScreen = Screen("COVID 19 MENU",600,400)
tempScreen = Screen("temp screen",400,300)
simScreen = Screen("COVID 19 SPREAD simulator",500,600,(0,0,0))
...
label = myfont.render("    Initial number of people- population: ",1, (255,255,0))
labelReady = Button(405,170,120,70,(255,0,0),"READY")
labelReturn = Button(150,170,70,50,(255,255,0),"RETURN")
labelCity = myfont.render("      Choose a real world city statistics> ",1, (255,255,0))
dropButton = Button(258,153,80,15,(40,255,40),"                               ")
myfont2 = pg.font.SysFont("Arial", 17)
labelSee = myfont2.render(" click to see",1, (0,0,0))
LabelGloves = myfont.render("                       Percent of worn gloves: ",1,(255,255,0))
LabelMasks = myfont.render("                       Percent of worn masks: ",1,(255,255,0))
LabelRespirator = myfont.render("         Number of available respirators: ",1,(255,255,0))
...
labelWrong = myfont2.render("CITY STATS NOT AVAILABLE",1,(255,255,0))
...
while running:
    ...
    if textBoxO.returnValue() == '':
        ...
        population = int(textBoxO.returnValue())
    ...
    if menuScreen.checkUpdate():
        ...
        if tempDropButton:
            ...
            for i in range(5):
                ...
                if i == 0:
                    ...
                    city = myfont.render("World",1,(255,255,0))
                    menuScreen.returnTitle().blit(city,(x,200))
                    x += 75
        ...
        if textBox1.returnValue() == "novi sad" or textBox1.returnValue() == "novisad":
            show_ns(globaltimer)

        if textBox1.returnValue() == "new york" or textBox1.returnValue() == "njujork":
            show_ny(globaltimer)

        if textBox1.returnValue() == "milano" or textBox1.returnValue() == "Milano":
            show_milano(globaltimer)

        if textBox1.returnValue() == "valjevo" or textBox1.returnValue() == "Valjevo":
            show_valjevo(globaltimer)

        if textBox1.returnValue() == "world" or textBox1.returnValue() == "svet":
            show_world(globaltimer)

        if not textBox1.returnValue() == "":
            menuScreen.returnTitle().blit(labelWrong,(237,80))
        ...
        labelSim = myfont2.render("Respirators: ",1,(255,255,255))
        simScreen.returnTitle().blit(labelSim,(230,551))
        
        labelSim = myfont3.render("Recovered: ",1,(255,255,255))
        ...
        if startButtonSim:
            ...
            for _ in range(population - 1):
                ...
                if textBoxObedient.returnValue() == "":
                    Obediance = 0
                else:
                    Obediance = int(textBoxObedient.returnValue())
